...
const ContactForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { ...formData, timestamp: new Date().toLocaleString(), source: 'Portfolio' };
    ...
    <section id="contact" className="py-24 scroll-mt-32">
      <div className="max-w-4xl mx-auto bg-[#181818] p-8 md:p-16 rounded-2xl border border-[#333] shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all hover:border-netflix-red relative overflow-hidden">
        ...
        <div className="text-center space-y-6 mb-16">
          <p className="text-netflix-red font-black uppercase tracking-[0.4em] text-xs">Strategic Partnerships</p>
          <h2 className="text-4xl md:text-6xl font-black tracking-tighter">Initiate Dialogue</h2>
          <p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto">
            Looking for a GTM engine to scale your technical product? Let's connect and define your next growth stage.
          </p>
        </div>
        ...
        {submitted ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-6 animate-fadeIn">
            ...
            <h3 className="text-3xl font-black">Transmission Received</h3>
            <p className="text-gray-400 text-lg text-center max-w-md">Raunak will reach out to you within 24 hours.</p>
            <button onClick={() => setSubmitted(false)} className="text-netflix-red font-black uppercase text-sm tracking-widest hover:underline pt-4">Back to form</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              <label className="text-xs font-black text-gray-500 uppercase tracking-widest">Full Name</label>
              <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full bg-[#252525] border border-transparent focus:border-netflix-red focus:bg-[#2a2a2a] focus:outline-none px-5 py-4 rounded-lg transition-all font-medium text-white" placeholder="Enter name" />
            </div>
            <div className="space-y-3">
              <label className="text-xs font-black text-gray-500 uppercase tracking-widest">Work Email</label>
            ...
            <div className="md:col-span-2 space-y-3">
              <label className="text-xs font-black text-gray-500 uppercase tracking-widest">Brief / Inquiry</label>
              <textarea required rows={4} value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} className="w-full bg-[#252525] border border-transparent focus:border-netflix-red focus:bg-[#2a2a2a] focus:outline-none px-5 py-4 rounded-lg transition-all resize-none font-medium text-white" placeholder="Outline your growth objectives..." />
            </div>
            ...
            <div className="md:col-span-2 pt-6">
              <button type="submit" disabled={isSubmitting} className={`w-full bg-netflix-red py-5 rounded-lg font-black text-xl uppercase tracking-[0.3em] hover:bg-[#f40612] hover:scale-[1.01] transition-all shadow-xl ${isSubmitting ? 'opacity-70' : ''}`}>
                {isSubmitting ? 'SUBMITTING...' : 'INITIATE CONTACT'}
              </button>
            </div>
          </form>
        )}
