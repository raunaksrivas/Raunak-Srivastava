    ...
    >
      <div className="aspect-video w-full rounded-lg overflow-hidden card-hover border border-transparent hover:border-[#333] bg-[#181818] shadow-2xl relative">
        <img 
          src={project.image} 
          alt={project.title} 
          className="w-full h-full object-cover transition-all duration-700 group-hover:scale-110 group-hover:opacity-20"
        />
        ...
        <div className="absolute inset-0 p-3 md:p-5 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-1 group-hover:translate-y-0 bg-gradient-to-t from-black via-black/80 to-transparent">
          <div className="space-y-1.5 md:space-y-2">
            ...
            <h3 className="font-bold text-base md:text-xl lg:text-2xl leading-tight drop-shadow-lg text-white line-clamp-2">
              {project.title}
            </h3>
            ...
            <button className="mt-2 w-full flex items-center justify-center gap-2 bg-white text-black py-1.5 md:py-2 rounded font-black text-[10px] md:text-xs hover:bg-white/80 transition-colors shadow-xl">
      ...
      {/* Visual Indicator Below (Hidden on hover for cleaner look) */}
